...
function add_sensor_noise(const_magic_nums, sensor_noise_dist, state, c, result)
    noisy_poses = similar(configuration(state))
    noisy_vels = similar(velocity(state))
    fill!(noisy_poses, 0.)
    fill!(noisy_vels, 0.)

    c.n_cache.rand_walks[1:3] .+= rand(sensor_noise_dist.gyro_rand_walk, 3)
    c.n_cache.rand_walks[4:6] .+= rand(sensor_noise_dist.accel_rand_walk, 3)

    last_noisy_R = Rotations.QuatRotation(c.n_cache.noisy_qs[1:4,end])
    add_arm_noise!(sensor_noise_dist, noisy_poses, noisy_vels, configuration(state)[8:end], c.n_cache.noisy_qs[8:end,end], 1/const_magic_nums.ctrl_freq)
    new_noisy_R = add_rotational_noise!(sensor_noise_dist, noisy_poses, noisy_vels, velocity(state)[1:3], last_noisy_R, 1/const_magic_nums.ctrl_freq, c.n_cache.rand_walks)
    add_linear_noise!(sensor_noise_dist, noisy_poses, noisy_vels, result.v̇[1:6], c.last_v̇, c.n_cache.noisy_vs[1:6,end], c.n_cache.noisy_qs[5:7,end], last_noisy_R, new_noisy_R, 1/const_magic_nums.ctrl_freq, c.n_cache.rand_walks)
...
function add_arm_noise!(sensor_noise_dist, noisy_poses, noisy_vels, joint_poses, last_noisy_joint_pose, dt)
    noisy_joint_poses = joint_poses + rand(sensor_noise_dist.arm_pos, length(joint_poses))
...
function add_rotational_noise!(sensor_noise_dist, noisy_poses, noisy_vels, body_vels, last_R, dt, rand_walk)
    noisy_vels[1:3] .= body_vels[1:3] + rand(sensor_noise_dist.v_ang_vel, 3) + rand_walk[1:3]
...
function add_linear_noise!(sensor_noise_dist, noisy_poses, noisy_vels, v̇, last_v̇, last_body_vels, last_pos, last_R, new_R, dt, rand_walk)
    noisy_linear_accels = last_v̇[4:6] + rand(sensor_noise_dist.accel, 3) + rand_walk[4:6]

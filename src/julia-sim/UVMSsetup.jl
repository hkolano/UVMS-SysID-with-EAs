function setup_frames(body_dict, body_name_list, cob_vec_dict, com_vec_dict, vehicle_extras_list, mvis)
    ...
    for (i, body_name) in enumerate(body_name_list)
        ...
        if i == vis_element && typeof(mvis) != UndefInitializer
    ...
    if haskey(body_dict, "jaw") == false
        ...
        if !(RigidBodyDynamics.is_fixed_to_body(body_dict["wrist"], jaw_com_frame))
            ...
            if typeof(mvis) != UndefInitializer

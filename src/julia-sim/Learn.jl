...
cob_vec_dict = Dict{String, SVector{3,Float64}}(dynamic_parameters["cob_vec_dict"])
buoyancy_mag_dict = Dict{String, Float64}(dynamic_parameters["buoyancy_mag_dict"])
link_volumes = dynamic_parameters["link_volumes"]
for (k,v) in dynamic_parameters["link_volumes"]
    buoyancy_mag_dict[k] = v*rho*9.81*0.001
end
com_vec_dict = Dict{String, SVector{3, Float64}}(dynamic_parameters["com_vec_dict"])
grav_mag_dict = Dict{String, Float64}(dynamic_parameters["grav_mag_dict"])
...
d_lin_coeffs = Vector{Float64}(dynamic_parameters["d_lin_coeffs"])
push!(d_lin_coeffs, d_lin_angular, d_lin_angular, d_lin_angular)
d_nonlin_angular = dynamic_parameters["d_nonlin_angular"]
d_nonlin_coeffs = dynamic_parameters["d_nonlin_coeffs"]
push!(d_nonlin_coeffs, d_nonlin_angular, d_nonlin_angular, d_nonlin_angular)

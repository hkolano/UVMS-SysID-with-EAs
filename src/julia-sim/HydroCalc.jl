...
# ------------------------------------------------------------------------
function hydro_calc!(com_frame_dict, cob_frame_dict, const_magic_nums, magic_num_pitch_val, magic_num_bluerov_with_alpha_arm, body_dict, joint_dict, hydro_wrenches::Dict{BodyID, Wrench{Float64}}, t, state::MechanismState, mvis)
    buoy_wrenches = []
    grav_wrenches = []
    ...
    for body_name in magic_num_pitch_val.body_names
        # Get the body
        ...
        buoy_force_trans = RigidBodyDynamics.transform(state, magic_num_bluerov_with_alpha_arm.blue_rov.buoyancy_force_dict[body_name], body_default_frame)
        ...
        grav_force_trans = RigidBodyDynamics.transform(state, magic_num_bluerov_with_alpha_arm.blue_rov.gravity_force_dict[body_name], body_default_frame)
        ...
        if print_now == true
            ...
            # @show transform(state, grav_wrench, root_frame(state.mechanism))
            @show RigidBodyDynamics.transform(state, buoy_wrench, default_frame(magic_num_bluerov_with_alpha_arm.blue_rov.body_dict["vehicle"]))
            @show RigidBodyDynamics.transform(state, grav_wrench, default_frame(magic_num_bluerov_with_alpha_arm.blue_rov.body_dict["vehicle"]))
        end
        ...
        if body_name == "wrist" && haskey(body_dict, "jaw") == false
            ...
            buoy_force_trans_wristdef = RigidBodyDynamics.transform(state, magic_num_bluerov_with_alpha_arm.blue_rov.buoyancy_force_dict["jaw"], body_default_frame)
            grav_force_trans_wristdef = RigidBodyDynamics.transform(state, magic_num_bluerov_with_alpha_arm.blue_rov.gravity_force_dict["jaw"], body_default_frame)
        ...
        if body_name == "vehicle"
            ...
            buoy_force_trans_armbase = RigidBodyDynamics.transform(state, magic_num_bluerov_with_alpha_arm.blue_rov.buoyancy_force_dict["armbase"], body_default_frame)
            grav_force_trans_armbase = RigidBodyDynamics.transform(state, magic_num_bluerov_with_alpha_arm.blue_rov.gravity_force_dict["armbase"], body_default_frame)
            ...
            # if @isdefined(vehicle_extras_list)
                for item_name in magic_num_bluerov_with_alpha_arm.blue_rov.vehicle_extras_list
                    # println("Applying forces to vehicle from "*item_name)
                    if haskey(magic_num_bluerov_with_alpha_arm.blue_rov.com_vec_dict, item_name)
                        def_to_center = fixed_transform(bod, body_default_frame, com_frame_dict[item_name])
                        force_trans = RigidBodyDynamics.transform(state, magic_num_bluerov_with_alpha_arm.blue_rov.gravity_force_dict[item_name], body_default_frame)

                    elseif haskey(magic_num_bluerov_with_alpha_arm.blue_rov.cob_vec_dict, item_name)
                        def_to_center = fixed_transform(bod, body_default_frame, cob_frame_dict[item_name])
                        force_trans = RigidBodyDynamics.transform(state, magic_num_bluerov_with_alpha_arm.blue_rov.buoyancy_force_dict[item_name], body_default_frame)
                    end
                    app_pt = Point3D(body_default_frame, translation(inv(def_to_center)))
                    if item_name == "foamL" && typeof(mvis) != UndefInitializer
            ...
            # @show vel
            tau_d = -magic_num_bluerov_with_alpha_arm.blue_rov.d_lin_coeffs .* vel .+ -magic_num_bluerov_with_alpha_arm.blue_rov.d_nonlin_coeffs .* vel .* abs.(vel)
        ...
        # ----- Drag on the links (quadratic only) ----
            ...
            F_d = transpose(-magic_num_bluerov_with_alpha_arm.link_drags[body_name]) .* abs.(cob_vel.v) .* cob_vel.v
